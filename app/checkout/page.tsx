...
// --- 1. RENAME COMPONENT (Remove 'export default' from here) ---
function CheckoutContent() {
  ...
  // Paystack processes amounts in Kobo. We multiply by 100.
  const config = {
  ...
  const onSuccess = async (reference: any) => {
      ...
      try {
          // A. Create array of enrollments for all items
          const enrollmentsToInsert = checkoutItems.map(item => ({
              user_id: user.id,
              course_id: item.id
          }))

          // B. Bulk Insert into Supabase
          const { error } = await supabase.from('enrollments').insert(enrollmentsToInsert)
          
          // Ignore 23505 (unique violation) if they somehow already own one of the courses
          if (error && error.code !== '23505') throw error

          // C. Clean up the main cart
...
// --- 2. DYNAMIC EXPORT (This fixes the Vercel 'window is not defined' error) ---
export default dynamic(() => Promise.resolve(CheckoutContent), { 
    ssr: false, // Disables server-side rendering for this specific file
    loading: () => (
        <div className="min-h-screen bg-[#050505] flex items-center justify-center">
            <Loader2 className="animate-spin text-emerald-500 w-10 h-10" />
        </div>
