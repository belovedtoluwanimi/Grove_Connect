...
type Step = 'intended-learners' | 'course-structure' | 'setup-test-video' | 'film-edit' | 'curriculum' 
      ...
      <aside className="w-72 border-r border-white/10 flex flex-col h-screen sticky top-0 shrink-0 z-10 bg-black/40 backdrop-blur-xl">
        <div className="p-6 border-b border-white/5">
          ...
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded border ${phase === 'plan' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}>
              Phase: {phase}
            </span>
          </div>
      ...
      <main className="flex-1 overflow-y-auto relative z-10">
        <div className="max-w-4xl mx-auto p-12 pb-32">
          <AnimatePresence mode="wait">
            ...
            {activeStep === 'curriculum' && <CurriculumStep key="s5" data={data} setData={setData} />}
          </AnimatePresence>
        </div>
      </main>
    </div>
...
// STEP 1: INTENDED LEARNERS (Maintained exactly as is)
function IntendedLearnersStep({ data, setData }: { data: CourseData, setData: any }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12">
      <div>
        <h1 className="text-4xl font-black mb-2">Intended Learners</h1>
            ...
            {data.objectives.map((obj, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex gap-3 group">
                <div className="flex-1 relative">
                  <input value={obj} onChange={e => handleObjChange(idx, e.target.value)} placeholder="Example: Build a full-stack Next.js application from scratch" className="w-full bg-zinc-900/80 border border-white/10 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-blue-500 transition-all text-white placeholder-zinc-700" />
                </div>
...
// STEP 2: COURSE STRUCTURE (Maintained exactly as is)
function CourseStructureStep() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12">
      <div>
        <h1 className="text-4xl font-black mb-2">Course Structure & Guidelines</h1>
...
// STEP 3: SETUP & TEST VIDEO (With new "Continue to Create Content" button)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-black/40 border border-white/5 rounded-3xl p-8 backdrop-blur-md space-y-6">
           ...
           <ul className="space-y-4">
              <li className="flex gap-4"><div className="p-2 bg-blue-500/10 text-blue-400 rounded-lg h-fit"><Mic size={18}/></div><div><h4 className="font-bold text-sm text-white">Invest in Audio</h4><p className="text-xs text-zinc-500">Students will forgive bad video, but bad audio will ruin a course. Get a dedicated USB or Lapel mic.</p></div></li>
              ...
              <li className="flex gap-4"><div className="p-2 bg-blue-500/10 text-blue-400 rounded-lg h-fit"><FileVideo size={18}/></div><div><h4 className="font-bold text-sm text-white">Keep it short</h4><p className="text-xs text-zinc-500">Break complex topics into bite-sized 3-7 minute videos. This increases completion rates dramatically.</p></div></li>
           </ul>
        </div>
        ...
        <div className="bg-black/40 border border-white/5 rounded-3xl p-8 backdrop-blur-md space-y-6">
           ...
           <div className="grid grid-cols-1 gap-3">
              <a href="#" className="p-4 bg-zinc-900/50 border border-white/10 rounded-xl flex items-center justify-between hover:border-purple-500/50 transition-colors group">
            ...
            <AnimatePresence>
              {mod.isOpen && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="p-5 space-y-4">
                  ...
                  <button onClick={() => addItem(mIdx)} className="w-full py-4 border-2 border-dashed border-white/10 rounded-xl text-sm font-bold uppercase tracking-wider text-blue-500 hover:bg-blue-500/10 hover:border-blue-500/30 transition-all flex items-center justify-center gap-2">
...
// --- 5. DEEP FUNCTIONAL CONTENT BUILDER (Videos, Slides, Articles, Quizzes, Resources) ---
      ...
      {item.isOpen && (
        <div className="border-t border-white/5 bg-zinc-900/30">
            ...
            <div className="flex px-6 pt-4 border-b border-white/5 text-xs font-bold uppercase tracking-wider">
                <button onClick={()=>setActiveTab('content')} className={`pb-3 px-4 border-b-2 transition-colors ${activeTab === 'content' ? 'border-blue-500 text-blue-400' : 'border-transparent text-zinc-500 hover:text-white'}`}>Build Content</button>
                <button onClick={()=>setActiveTab('resources')} className={`pb-3 px-4 border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'resources' ? 'border-blue-500 text-blue-400' : 'border-transparent text-zinc-500 hover:text-white'}`}>Resources <span className="bg-white/10 px-1.5 py-0.5 rounded-full text-[10px]">{item.resources?.length || 0}</span></button>
            </div>
                    ...
                    <div className="animate-in fade-in">
                        ...
                        {item.type === 'article' && (
                            <div className="space-y-2">
                                ...
                                <textarea value={item.content || ''} onChange={e=>updateItem('content', e.target.value)} placeholder="Start writing your article here..." className="w-full h-64 bg-black/50 border border-white/10 rounded-xl p-4 text-sm outline-none focus:border-blue-500 focus:bg-black resize-none text-zinc-300 transition-all" />
                            </div>
                        )}
                        
                        {/* Quiz & Practice Test Builder */}
...
// --- SUB-EDITORS FOR SPECIFIC TYPES ---
...
function VideoUploader({ label = "Upload Video", url, onUpload }: any) {
    ...
    return (
        <label className="flex flex-col items-center justify-center w-full aspect-video bg-black/60 border border-dashed border-white/20 rounded-xl cursor-pointer hover:border-blue-500 hover:bg-black/80 transition-all overflow-hidden group">
            {url ? <video src={url} controls className="w-full h-full object-cover"/> : (
                <div className="text-center">
                    {uploading ? <Loader2 className="animate-spin text-blue-500 mx-auto mb-3" size={28}/> : <UploadCloud className="text-zinc-500 group-hover:text-blue-400 mx-auto mb-3 transition-colors" size={32}/>}
            ...
            {quizData.map((q, qIdx) => (
                <div key={q.id} className="bg-black/40 border border-white/10 rounded-2xl p-6 relative group">
                    ...
                    <div className="flex gap-4 mb-6 pr-8">
                        <span className="w-8 h-8 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center text-sm font-black shrink-0">{qIdx + 1}</span>
                        <div className="flex-1 space-y-1">
                            <label className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider">Question Prompt</label>
                            <input value={q.question} onChange={e=>updateQ(qIdx, 'question', e.target.value)} placeholder="Type your question here..." className="w-full bg-transparent border-b border-white/20 pb-2 outline-none font-medium text-white focus:border-blue-500 text-lg placeholder-zinc-700 transition-colors" />
                        </div>
                    </div>
                    ...
                    <div className="pl-12 space-y-3">
                        ...
                        {q.options.map((opt, oIdx) => (
                            <div key={oIdx} className="flex items-center gap-3">
                                <input type="radio" name={`correct-${q.id}`} checked={q.correctIndex === oIdx} onChange={() => updateQ(qIdx, 'correctIndex', oIdx)} className="w-5 h-5 accent-blue-500 cursor-pointer" title="Mark as correct answer" />
                                <input value={opt} onChange={e=>updateOpt(qIdx, oIdx, e.target.value)} placeholder={`Option ${oIdx + 1}`} className={`flex-1 bg-zinc-900/80 border rounded-xl px-4 py-2.5 text-sm outline-none transition-colors ${q.correctIndex === oIdx ? 'border-blue-500/50 text-blue-100 bg-blue-500/5' : 'border-white/5 focus:border-white/30 text-white placeholder-zinc-600'}`} />
                        ...
                        {q.options.length < 5 && <button onClick={()=>addOpt(qIdx)} className="text-xs font-bold text-blue-500 hover:text-blue-400 flex items-center gap-1 mt-2 px-1"><Plus size={14}/> Add Another Option</button>}
                    </div>
                </div>
            ))}
            ...
            <div className="space-y-3">
              ...
              {resources.map((r, i) => (
                  <div key={r.id} className="flex items-center gap-4 bg-black/60 p-4 rounded-xl border border-white/10 group hover:border-white/20 transition-colors">
                      ...
                      <input value={r.title} onChange={e => {const n=[...resources]; n[i].title=e.target.value; onChange(n)}} placeholder="Resource Title (e.g. Cheat Sheet)" className="bg-transparent text-sm w-1/3 outline-none font-bold placeholder-zinc-700" />
