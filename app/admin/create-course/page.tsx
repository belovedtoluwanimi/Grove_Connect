...
import { CourseData, INITIAL_DATA, INITIAL_AVAILABILITY, Step, CATEGORIES, STANDARD_PRICES, PREMIUM_PRICES, InstructorVerification } from './types'
import { 
...
export default function CourseStudioPage() {
  ...
  // Load draft
  useEffect(() => {
    const saved = localStorage.getItem('grove_course_draft_v2')
    if (saved) {
  ...
  useEffect(() => {
    if (data.mode) {
      const t = setTimeout(() => {
        localStorage.setItem('grove_course_draft_v2', JSON.stringify(data))
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      localStorage.removeItem('grove_course_draft_v2')
    ...
    <div className="min-h-screen bg-[#050505] text-white flex font-sans selection:bg-emerald-500/30">
      ...
      <aside className="w-72 bg-[#0a0a0a] border-r border-white/10 flex flex-col p-6 sticky top-0 h-screen shrink-0 z-20">
        ...
        <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between text-[10px] text-zinc-500 font-mono">
           ...
           <button onClick={() => { localStorage.removeItem('grove_course_draft_v2'); setData(INITIAL_DATA); setStep('mode') }} className="hover:text-red-400">Reset</button>
        </div>
      </aside>
          ...
          {step === 'details' && (
             <div className="space-y-8 animate-in fade-in">
                ...
                <div className="space-y-2">
                   ...
                   {data.category === 'Other' && <Input value={data.customCategory} onChange={(v: string) => setData({...data, customCategory: v})} placeholder="Specify category..." />}
                </div>
             ...
             <div className="space-y-8 animate-in fade-in">
                ...
                <div className="flex gap-4">
                   {(data.mode === 'premium' ? PREMIUM_PRICES : STANDARD_PRICES).map(p => (
                      <button key={p} onClick={() => setData({...data, pricing: { ...data.pricing, amount: p === 'Custom' ? '' : p }})} className={`px-6 py-4 rounded-xl border ${data.pricing.amount === p ? 'bg-white text-black' : 'border-white/10 text-zinc-400'}`}>{p === 'Custom' ? 'Custom' : `$${p}`}</button>
                   ))}
                </div>
          ...
          {step === 'review' && (
             <div className="space-y-8 animate-in fade-in">
                ...
                <div className="bg-zinc-900 border border-white/10 rounded-xl p-6 text-sm space-y-4">
                    <ReviewRow label="Title" value={data.title} active={!!data.title} />
                    <ReviewRow label="Price" value={`$${data.pricing.amount}`} active={!!data.pricing.amount} />
                    <ReviewRow label="Status" value={isVerified ? 'Verified' : 'Pending'} active={isVerified} />
                </div>
