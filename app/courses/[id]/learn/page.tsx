        ...
        const init = async () => {
            try {
                ...
                let { data: courseData, error } = await supabase.from('courses')
                    .select('*, profiles:instructor_id(full_name, avatar_url, bio, social_twitter, social_linkedin, social_instagram)')
            ...
            <p className="text-emerald-400 text-sm font-bold mb-4">Course Educator</p>
                                            ...
                                            <div className="flex items-center gap-4 mt-4">
                                                {instructor?.social_twitter && (
                                                    <a href={`https://twitter.com/${instructor.social_twitter.replace('@', '')}`} target="_blank" rel="noreferrer" className="text-zinc-400 hover:text-blue-400 transition-colors"><Twitter size={18} /></a>
                                                )}
                                                {instructor?.social_linkedin && (
                                                    <a href={instructor.social_linkedin.includes('http') ? instructor.social_linkedin : `https://linkedin.com/in/${instructor.social_linkedin}`} target="_blank" rel="noreferrer" className="text-zinc-400 hover:text-blue-600 transition-colors"><Linkedin size={18} /></a>
                                                )}
                                                {instructor?.social_instagram && (
                                                    <a href={`https://instagram.com/${instructor.social_instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="text-zinc-400 hover:text-pink-500 transition-colors"><Instagram size={18} /></a>
                                                )}
                                            </div>
                                        </div>
                                    </div>
